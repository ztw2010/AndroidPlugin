...
public class Vitamio {
      ...
      if (libs != null) {
        ...
        for (String L : getRequiredLibs()) {
          if (Arrays.binarySearch(libs, L) < 0) {
            com.small.test.appstub.log.L.e("Native libs %s not exists!", L);
            return false;
          }
        }
        ...
        try {
          ...
          L.i("isNativeLibsInited, APP VERSION: %d, Vitamio Library version: %d", appVersion, libVersion);
          if (libVersion == appVersion)
            return true;
        } catch (IOException e) {
          L.e("isNativeLibsInited", e);
  ...
  private static boolean extractLibs(Context ctx, int rawID) {
    ...
    L.d("loadLibs start " + version);
    File lock = new File(getLibraryPath() + LIBS_LOCK);
    if (lock.exists())
      lock.delete();
    String libPath = copyCompressedLib(ctx, rawID, "libarm.so");
    L.d("copyCompressedLib time: " + (System.currentTimeMillis() - begin) / 1000.0);
    ...
    try {
      ...
      L.e("Error creating lock file", e);
    } finally {
      L.d("initializeNativeLibs: " + inited);
      L.d("loadLibs time: " + (System.currentTimeMillis() - begin) / 1000.0);
